...
import pickerTpl from './picker.tpl';
...
class Picker extends Base {
    ...
    parse() {
        ...
        let $picker = $($.render(pickerTpl, options));
        this.$body = $picker;
        const depth = options.depth || (isMulti ? items.length : util.depthOf(items[0]));
        this.depth = depth;
        let groups = '';
        ...
        let me = this;
        $picker
            .on('click', '.weui-mask', function () { me.hide(); })
            .on('click', '.weui-picker__action', function () { me.hide(); })
            .on('click', '#weui-picker-confirm', function () {
                options.onConfirm(me.result);
            });
    ...
    scroll(items, level) {
        ...
        let lineTemp = this.temp[options.id] || [];
        let $picker = this.$body;
        let result = this.result;
        let isMulti = this.isMulti;
        let depth = this.depth;
        if (lineTemp[level] === undefined && options.defaultValue && options.defaultValue[level] !== undefined) {
            // 没有缓存选项，而且存在defaultValue
            const defaultVal = options.defaultValue[level];
            ...
            onChange: function (item, index) {
                ...
                if (isMulti) {
                    ...
                    if (item.children && item.children.length > 0) {
                        ...
                        !isMulti && me.scroll(item.children, level + 1); // 不是多列的情况下才继续处理children
                    } else {
...
class DatePicker extends Picker {
    ...
    val() {
        ...
        if (arguments.length) {
            ...
            this.$body.find('.weui-input').text(pickerLabel);
